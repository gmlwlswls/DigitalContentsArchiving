  ...
  def __renameHelp(self, product_type_path, product_name):
      ...
      for root, _, files in os.walk(product_type_path):
          ...
          for file in sorted(files):
              ...
              # 중복 방지를 위한 기본 이름
              new_name_base = f"{product_name}{volume_suffix}_{folder_name}_{mod_time}{dozen_suffix}{country_suffix}"
              new_name_base_key = f"{new_name_base}{ext}"

              # 중복 방지용
              count = name_count_dict.get(new_name_base_key, 1)
              while True :
                  count_suffix = f"_{count}" if count > 1 else ""
                  new_file_name = f"{doc_number}_{new_name_base}{count_suffix}{ext}"
    ...
    for product_line in os.listdir(self.base_path):
        ...
        # ProductLine 폴더 내 ProductName 폴더 찾기
            ...
            for product_name in os.listdir(product_line_path):
                overseas_folderlist = ['1_NorthAmerica_북미', '2_Europe_유럽', '4_OverseasOther_해외기타']           
